...
def main():
    ...
    generator_opt = tf.keras.optimizers.Adam(learning_rate=0.0003, beta_1=0.5)
    ...
    for i in range(num_epochs):
        ...
        if (i+1) % visualization_step == 0:
            plt.xlabel("Epochs")
            plt.ylabel("Loss")
            plt.plot(generator_losses_epoch, 'blue', label="Generator Loss")
            plt.plot(discriminator_losses_epoch, 'red', label="Discriminator Loss")
